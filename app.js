...
function saveState(mochis) {
  try {
    ...
    console.log("saveState error:", e?.message || e);
  }
}
...
function normalizeMochi(m) {
  ...
  m.lastActive = Number.isFinite(m.lastActive) ? m.lastActive : t;

  m.dreamText = m.dreamText ? String(m.dreamText) : "";
  if (hasCjk(m.dreamText)) m.dreamText = "";

  m.toppings = Array.isArray(m.toppings) ? m.toppings : [];

  m.isEaten = !!m.isEaten;
  m.eatScale = Number.isFinite(m.eatScale) ? m.eatScale : 1.0;
...
function findSpot() {
  ...
  return { x: rand(150, 1050), y: rand(150, 650) };
}
...
function stepDream(m, now) {
  ...
  if (now - m.lastActive > 8000) {
    m.status = "DAYDREAM";
    if (!m.dreamText || hasCjk(m.dreamText)) m.dreamText = pick(dreamLines);
    markDirty();
  }
}
...
function stepFerment(m, now) {
  ...
  const t = (now - (m.fermentStart || now)) / 1000;
  const p = Math.min(t / fermentSec, 1);
  const s = smooth01(p);

  m.sizeW = lerp(100, 145, s);
  m.sizeH = lerp(80, 120, s);
...
function liveCursors() {
  ...
  for (const id in cursors) {
    const c = cursors[id];
    if (c && now - (c.ts || 0) < cursorTtlMs) out[id] = c;
  }
  return out;
}

function broadcastState() {
  io.emit("state", { mochis, cursors: liveCursors() });
}
...
function applyAction(m, type, data) {
  ...
  if (type === "start_knead") {
    if (m.status === "IDLE" || m.status === "DAYDREAM") m.isKneading = true;
    return;
  }
  ...
  if (type === "finish_bake_click") {
    if (m.status === "BAKING") m.status = "BAKED";
    return;
  }

  if (type === "eat_click") {
    if (m.status === "BAKED") m.isEaten = true;
  }
}

io.on("connection", (socket) => {
  cursors[socket.id] = { x: 0, y: 0, ts: nowMs() };
  socket.emit("state", { mochis, cursors: liveCursors() });
